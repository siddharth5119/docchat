...
def get_document_text(doc):
    ...
    if file_type == '.pdf':
        ...
        for i, para in enumerate(docx_reader.paragraphs):
            text += f"[Paragraph {i+1}] {para.text}\n"
...
def user_input(user_question):
    ...
    # Collect references for each document with clickable links
    references = []
    for doc in docs:
        document_name = doc.metadata.get('document_name', 'Unknown')
        if 'page_number' in doc.metadata:
            references.append(f"[{document_name} (Page {doc.metadata['page_number']})](#{doc.metadata['document_name']}_page_{doc.metadata['page_number']})")
        elif 'slide_number' in doc.metadata:
            references.append(f"[{document_name} (Slide {doc.metadata['slide_number']})](#{doc.metadata['document_name']}_slide_{doc.metadata['slide_number']})")
        elif 'row_number' in doc.metadata:
            references.append(f"[{document_name} (Row {doc.metadata['row_number']})](#{doc.metadata['document_name']}_row_{doc.metadata['row_number']})")
...
def display_conversation():
    for idx, convo in enumerate(st.session_state['conversation']):
        ...
        st.markdown(f"_References:_ {', '.join(convo['references'])}")
        st.markdown("---")
...
if uploaded_files:
    document_references = {}  # To store document references
    ...
    for file in uploaded_files:
        ...
        if st.sidebar.button(f"Preview {file.name}"):
            ...
            document_references[file.name] = document_text  # Store document text for reference
            ...
            if file.type == "application/pdf":
                ...
                for i in range(len(pdf_reader.pages)):
                    references.append(f"[{file.name} (Page {i + 1})](#{file.name}_page_{i + 1})")
            elif file.type == "application/vnd.openxmlformats-officedocument.wordprocessingml.document":
                docx_reader = docx.Document(file)
                for i in range(len(docx_reader.paragraphs)):
                    references.append(f"[{file.name} (Paragraph {i + 1})](#{file.name}_para_{i + 1})")
            elif file.type == "application/vnd.openxmlformats-officedocument.presentationml.presentation":
                ppt = pptx.Presentation(file)
                for i in range(len(ppt.slides)):
                    references.append(f"[{file.name} (Slide {i + 1})](#{file.name}_slide_{i + 1})")
            elif file.type == "text/csv":
                csv_reader = csv.reader(file.read().decode("utf-8").splitlines())
                for i, row in enumerate(csv_reader):
                    references.append(f"[{file.name} (Row {i + 1})](#{file.name}_row_{i + 1})")

            # Combine text and references for preview
            preview_text = document_text + "\n\n_References:_ " + ", ".join(references)
            with st.expander(f"📄 Preview: {file.name} (Full Text)"):
                st.write(preview_text)  # Display full document preview
