...
def user_input(user_question, uploaded_files):
    ...
    # Create a mapping of document indices to file names
    file_names = [file.name for file in uploaded_files]
    
    # Collect references using custom numbering
    references = []
    
    for idx, doc in enumerate(docs):
        # Check if the document contains relevant content
        if "logistic regression" in doc.page_content.lower():
            # Ensure we don't go out of bounds for file_names
            if idx < len(file_names):
                references.append(f"Page No: {idx + 1} from Document: {file_names[idx]}")
            else:
                references.append(f"Page No: {idx + 1} from Unknown Document")

    # If references are empty, provide a no relevant documents found message
    if not references:
        references.append("No relevant documents found related to your question.")
    ...
    # Return the answer and references for display
    return response["output_text"], references
...
if uploaded_files:
    ...
    for idx, file in enumerate(uploaded_files):
        ...
        # Process and split text for vector storage
        combined_text += get_document_text(file) + f" Document: {file.name}"
